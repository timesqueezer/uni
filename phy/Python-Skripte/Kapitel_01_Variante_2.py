...
# Z[n+1] = r*Z[n]*(1-Z[n])
# In dieser zweiten Variante wird r mit Hilfe einer Schleife automatisch 
...
# verwendet. In der Animation entspricht jedes einzelne Bild einem r,
# das im Laufe der Animation durchlaufen wird.
...
fig = plt.figure()
ax = plt.axes(xlim=(0, num_points-1), ylim=(0, 1))
ax.set_xlabel('$n$')
ax.set_ylabel('$Z_n$')
line, = ax.plot([], [], ".")
...
def update(i):
    line.set_data(n_werte, Z[i,:])
    return line,

ani = animation.FuncAnimation(fig, update, init_func=init,
                               frames=num_r, interval=20)
