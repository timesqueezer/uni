...
class Stoppuhr
{
    ...
    public void stopp()
    {
        ...
        printWarningsForShortIntervals(ms);
        _start = System.nanoTime();
    }

    /**
     * Gibt fuer kurze Messintervalle eine entsprechende Warnung aus.
     */
    private void printWarningsForShortIntervals(long ms)
    {
        if (ms < 100)
        {
            _out.println("*** Messintervall ist sehr kurz ***");
        }
        else if (ms < 1000)
        {
            _out.println("*** Messintervall ist recht kurz ***");
        }
    }
}
